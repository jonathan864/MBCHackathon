...
function getWalletClient() {
  ...
  const privateKey =
    "5ee91e6af2252535862fd1d8daedafc0dcc091f89a181d93556bc371bc85d5a9";
...
export async function logEvaluationOnchain(params: {
  ...
  try {
    ...
    console.log(
      "AgentGuard logger wallet address:",
    ...
    if (
      ...
      lowerErrorMessage.includes("gas")
    ) {
      throw new Error(
        "Onchain logging failed: insufficient funds on the logging wallet"
      );
    }

    if (
      lowerErrorMessage.includes("invalid private key") ||
      lowerErrorMessage.includes("bad key")
    ) {
